...
ENV_HPC_HF_TEI_PATH = Path('.env.hpc.hf.tei')
...
class HuggingFaceCLI:
    ...
    def download_model(model_hub_id: str):
        ...
        cmd = (
            ...
            '--nv '
            f'--bind {bind} '
            f'--env-file {ENV_HPC_HF_TEI_PATH} '
...
class TEIServerInstance:
    ...
    def start(mount_path: Path):
        ...
        logger.info('Waiting for TEI server to become healthy...')
        ...
        while True:
            try:
                ...
                if response.status == 200:
                    logger.info('TEI server is healthy.')
                    break
...
class TEIClient:
    ...
    def run(batch_request_id: UUID):
        env = os.environ.copy()
        env['TEI_BASE_URL'] = TEI_BASE_URL
        env['BATCH_REQUEST_ID'] = str(batch_request_id)

        cmd = 'apptainer run ' f'--env-file {ENV_HPC_HF_TEI_PATH} ' f'{CLIENT_SIF_PATH}'
        subprocess.run(cmd, check=True, capture_output=False, shell=True, env=env)
...
def process_batch_request(
    ...
    while True:
        ...
        if not previous_batch_job:
            TEIServerInstance.start(mount_path)
            is_server_instance_running = True

        elif previous_batch_job.model_hub_id != batch_job.model_hub_id:
            TEIServerInstance.stop()
            TEIServerInstance.start(mount_path)
            is_server_instance_running = True

        TEIClient.run(batch_job.batch_request_id)
