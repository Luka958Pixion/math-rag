...
class SSHClient:
    ...
    async def connect(self) -> SSHClientConnection:
        return await connect(self.host, username=self.user, client_keys=[self.private_key])
    ...
    async def run(self, command: str) -> str:
        async with await self.connect_retry() as connection:
            ...
            logger.info(f'Command `{command}` in `{self.run.__name__}` returned stdout: {stdout}')
